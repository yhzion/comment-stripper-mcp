 ...
 */
function replaceLiterals(code: string, pattern: RegExp, literalMap: Map<string, string>, prefix: string): string {
  let lastIndex = 0;
 ...
 */
function restoreLiterals(code: string, literalMap: Map<string, string>): string {
  if (literalMap.size === 0) return code;
 ...
 * @returns The code with all comments removed.
 */
export function stripCStyleComments(code: string): string {
 ...
 * @returns The code with all comments removed.
 */
export function stripPhpComments(code: string): string {
  if (!code) return '';
...
const FILE_TYPE_CACHE = new Map<string, (code: string) => string>();
 ...
 */
export function stripCommentsByFileType(filePath: string, code: string): string {
  if (!code) return '';
 ...
 * @returns The code with all comments removed.
 */
export function stripComments(code: string, fileType?: string): string {
  if (!code) return '';
