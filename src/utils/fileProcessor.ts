 ...
 */
export async function processFile(filePath: string): Promise<{ original: string; stripped: string }> {
  try {
 ...
 */
export function shouldProcessFile(filePath: string, fileTypes: string[] = SUPPORTED_EXTENSIONS): boolean {
  const extension = path.extname(filePath).slice(1).toLowerCase();
...
async function countFiles(
  ...
  fileTypes: string[] = SUPPORTED_EXTENSIONS
): Promise<number> {
  let count = 0;
...
): Promise<{ [filePath: string]: { original: string; stripped: string } }> {
  const results: { [filePath: string]: { original: string; stripped: string } } = {};
